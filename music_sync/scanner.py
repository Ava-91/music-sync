...
def scan_library(root: str | Path, side: Side, progress: Callable[[int], None] | None = None, cancel: Callable[[], bool] | None = None) -> ScanResult:
    ...
    if not root.exists(): result.errors.append(f"Directory does not exist: {root}"); return result
    if not root.is_dir(): result.errors.append(f"Not a directory: {root}"); return result
    try:
        paths = list(root.rglob("*"))
    except OSError as exc:
        result.errors.append(f"Could not enumerate {root}: {exc}"); return result
