...
def test_a_only_requires_explicit_copy_decision(tmp_path: Path):
    ...
    result = execute_reconcile(
        fresh_plan(a, b, library_a_only=[Track(track, "a")]),
        {f"a-only:{track}": ReconcileDecision.COPY_A_TO_B},
        backups,
    )
...
def test_conflict_keep_a_keep_b_and_skip_are_explicit(tmp_path: Path):
    cases = [
        ...
        (ReconcileDecision.KEEP_B, b"b", b"b"),
    ...
    for decision, expected_a, expected_b in cases:
        ...
        match = Match(
            Track(a_track, "a", artwork_hash="a"),
            Track(b_track, "b", artwork_hash="b"),
            0.9,
            artwork_conflict=True,
        )
